...
import cv2
import time
from ultralytics.value import MeterDetection
from ultralytics.reading import Basic_processing
...
def value():
    try:
        image01 = cv2.imread('../video/888.jpg')  # 测试图片
        image01 = cv2.resize(image01, (480, 640))
        image02 = cv2.imread('../video/1.jpg')  # 模板图片
...
class Results(SimpleClass):
    ...
    def plot(
        ...
        if pred_boxes is not None and show_boxes:
            # 添加数据
            t = threading.Thread(target=value)  # 线程
            t.daemon = True  # 设置为守护线程
            t.start()
            for d in reversed(pred_boxes):
                c, conf, id = int(d.cls), float(d.conf) if conf else None, None if d.id is None else int(d.id.item())
                name = ("" if id is None else f"id:{id} ") + names[c]
                label = (f"{name} {conf:.2f}MPa:{value_list[0]}" if conf else name) if labels else None  #添加自己的标签
